...
Modules are managed in IaSQL through the PostgreSQL functions loaded into every IaSQL database. As the [Tutorial](/typeorm) guide indicates, you pick which modules you wish to use once you connect to your hosted IaSQL DB. You can cofigure which modules are installed over time using the `SELECT * FROM iasql_install(...)` or ` SELECT * FROM iasql_uninstall(...)` commands. See the [IaSQL PostgreSQL Functions](/function) for more details.