...
Modules are managed in IaSQL through the `iasql` CLI. As the [Quickstart](/quickstart) guide indicates, you are prompted when first setting up a hosted IaSQL DB which modules you wish to use, but you can modify that at any time using the `iasql install` or `iasql uninstall` commands. See the [CLI Reference](/cli) for more details.